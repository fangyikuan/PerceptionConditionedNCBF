...
class CBF_QP_Filter:
    ...
    * Works with any torch nn.Module producing scalar h(x).
    ...
    def safe_action(self, states:np.ndarray, nn_input:np.ndarray, diff_perception:np.ndarray):
        ...
        u_nom = u_nom[::-1,:].copy() # switch angular and linear
        ...
        u_nom = u_nom.reshape(-1) #  linear acc, angular vel
        ...
        u_corr = u.value.astype(np.float32)[::-1]
        return u_corr
        # return u_nom.astype(np.float32)
if __name__ == "__main__":
    ...
    train = False
    load_dir = "./cbf_model_epoch_16.pt"
    ...
    if train:
        dataset_dict = build_dataset_from_env(env_kwargs, num_traj=1e7, horizon=1000,
                                              save_path="./env_dataset_1e7.pkl",
                                              dump=True,
